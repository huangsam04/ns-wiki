...
```mermaid
graph TD
    A["接收 USB 主机请求"] --> B["接收 RCM 消息和 Payload"]
    B --> C["检查 RCM 消息合法性"]
    C -->|"不合法"| D["返回, 不执行 Payload"]
    C -->|"合法"| E["保存用户代码地址并跳转执行"]
```
...
<details>
  ...
  <div class="mermaid">
graph TD
    A("主机与Switch建立USB通信") --> B("bootROM接收RCM消息")
    B --> B2("RCM Payload写入RCM区")
    B2 --> C("主机发送GET_STATUS")
    C --> D("主机指定wLength")
    D --> E("bootROM拷贝栈中wLength字节到USB DMA Buffer2")
    E --> F{"wLength > 2 ?"}
    F -->|否| G("仅拷贝2字节 返回正常")
    F -->|是| H("拷贝超出Buffer2范围")
    H --> I("RCM Payload数据覆盖栈区")
    I --> J{"返回指针被覆盖?"}
    J -->|否| K("未获得控制权 可能崩溃")
    J -->|是| L("返回指针跳转到Payload代码")
    L --> M("获得RCM权限 执行任意代码")
