---
title: 书写说明
description: 本指南旨在规范Wiki编辑行为，确保内容合规与结构统一。编辑前需联系管理员获取正式权限，并遵守网络资源法律免责声明。内容创建需遵循结构规范：页面路径须使用英文且无空格，标题填写中文，简短说明留空由系统生成，禁止填写脚本样式，文章仅限一个一级标题以优化搜索索引，素材需上传至指定目录并调整尺寸。
published: true
date: 2026-02-14T13:04:40.184Z
tags: 
editor: markdown
...
### 脚本 样式 需要遵循的标准
不能填写脚本与样式，是因为本站使用的自动生成描述程序在替换描述的时候会把脚本和样式一块扬了，目前不知道怎么修复。
